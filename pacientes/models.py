...
class Paciente(Persona):
    ...
    grupo_sanguineo = models.CharField(
        max_length=20,
        null=True,
        choices=Choices('0-', '0+', 'A-', 'A+', 'B-', 'B+', 'AB-', 'AB+')
    )
...
class HistoriaClinica(models.Model):
    ...
    paciente = models.ForeignKey(
        'Paciente', related_name='historial_clinico',
    ...
    def __str__(self):
        return '{} - {}, {}'.format(
            self.id, self.paciente.nombres, self.paciente.apellido
        )
...
class Consulta(TimeStampedModel):
    ...
    # Podría ser un manytomany a un modelo de medicamentos
    ...
    def __str__(self):
        return '{} - fecha: {} - paciente: {}'.format(
            self.id, self.fecha, self.historia.paciente
        )
